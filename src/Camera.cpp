...
namespace RenderEngine {
 ...
 void Camera::truck(double value) noexcept {
    position_ += vec3{value, 0, 0} * orientation_;
  }
 void Camera::pedestal(double value) noexcept {
    position_ += vec3{0, value, 0} * orientation_;
  }
 void Camera::dolly(double value) noexcept {
    position_ += vec3{0, 0, value} * orientation_;
  }

 void Camera::tilt(double degrees) noexcept {
 ...
 void Camera::pan(double degrees) noexcept {
    auto const radians = degrees * (std::numbers::pi / 180.);
    const double SIN = std::sin(radians);
    const double COS = std::cos(radians);
    orientation_ =
 ...
 void Camera::roll(double degrees) noexcept {
    auto const radians = degrees * (std::numbers::pi / 180.);
    const double SIN = std::sin(radians);
    const double COS = std::cos(radians);
    orientation_ =
