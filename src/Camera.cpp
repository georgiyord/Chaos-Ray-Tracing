...
Camera::Camera(vec3 position, Matrix3x3 orientation) noexcept
    : position_(position), orientation_(orientation) {}
Camera::Camera() noexcept
    : position_(vec3::zero()), orientation_(Matrix3x3::one()) {}
...
void Camera::updatePosition(const vec3 &value) noexcept { position_ = value; }
...
void Camera::pan(float degrees) noexcept {
  ...
  const float SIN = std::sin(radians);
  const float COS = std::cos(radians);
  orientation_ = Matrix3x3{{COS, 0, -SIN, 0, 1, 0, SIN, 0, COS}} * orientation_;
}
void Camera::roll(float degrees) noexcept {
  auto const radians = degrees * (std::numbers::pi_v<float> / 180.f);
  const float SIN = std::sin(radians);
  const float COS = std::cos(radians);
  orientation_ = Matrix3x3{{COS, SIN, 0, -SIN, COS, 0, 0, 0, 1}} * orientation_;
}
} // namespace RenderEngine